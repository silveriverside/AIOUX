...
- `server/snapshots.js` 的 `commitNodeAsync()` 只串行化了 git commit，但 `writeNodeHtml()` 在入队前执行；连续提交同一节点时可能出现前一个任务提交后一个 HTML 的串版本问题。后续修复分支：`fix/snapshot-queue-atomicity`。
...
- 按分支协作工作流先修复快照异步队列原子性：把“写 HTML + git add + commit”作为同一串行临界区，并补充同节点连续 `/api/sync` 回归测试。
