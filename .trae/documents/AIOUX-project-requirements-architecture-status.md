...
> 说明：本文是项目需求与架构演进历史账本，部分早期“当前”描述保留历史上下文；最新状态以 `AIOUX-global-progress.md` 与 `AIOUX-roadmap-current.md` 为准。
...
- `public/index.html` 中 sandbox iframe 曾同时允许 `allow-scripts` 和 `allow-same-origin`，而生成页面允许内联脚本；这会削弱隔离边界。该项后续已阶段性处理：默认 sandbox 已移除 `allow-same-origin`，iframe bridge 已加入 source/nonce/payload 白名单校验，并由 E2E 覆盖；是否进一步禁用或收紧内联脚本仍需单独策略确认。
