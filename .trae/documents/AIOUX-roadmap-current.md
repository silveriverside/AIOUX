...
- 最新验证：`npm run e2e` 已默认启动隔离服务并使用临时 snapshots；意图路由评估器当前 22/22 通过；素材质量评估器已可离线输出排序报告；近期全量 `node --test` 为 126/126 通过，`GetDiagnostics` 无错误。
...
- 素材质量：`npm run eval:asset-quality` 应输出素材总数、Top 素材和质量分；`--json` 应输出按质量分排序的 assets 列表；`--limit=N` / `--limit N` 应限制输出条数但保留全量 total；无效 limit 应显式失败。
